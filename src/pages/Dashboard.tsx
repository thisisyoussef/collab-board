...
type DashboardSearchState = Record<DashboardView, string>;
...
interface PersistedDashboardViewPayload {
  activeView: DashboardView;
  searchByView: DashboardSearchState;
}

function getStorageApi(): Storage | null {
...
function parsePersistedDashboardView(): PersistedDashboardViewPayload | null {
  ...
  try {
    const raw = storage.getItem(DASHBOARD_VIEW_STORAGE_KEY);
    if (!raw) {
      return null;
    }
    ...
    const parsed = JSON.parse(raw) as {
      activeView?: unknown;
      searchByView?: Partial<Record<DashboardView, unknown>>;
    };
    const activeView: DashboardView = parsed.activeView === 'shared' ? 'shared' : 'owned';

    return {
      activeView,
      searchByView: {
        owned: typeof parsed.searchByView?.owned === 'string' ? parsed.searchByView.owned : '',
        shared: typeof parsed.searchByView?.shared === 'string' ? parsed.searchByView.shared : '',
      },
    };
  } catch {
    return null;
...
function persistDashboardView(payload: PersistedDashboardViewPayload): void {
  const storage = getStorageApi();
  if (!storage) {
    return;
  }
  ...
  try {
    storage.setItem(DASHBOARD_VIEW_STORAGE_KEY, JSON.stringify(payload));
  } catch {
...
export function Dashboard() {
  ...
  const [activeView, setActiveView] = useState<DashboardView>(
    () => parsePersistedDashboardView()?.activeView ?? 'owned',
  );
  ...
  const [searchByView, setSearchByView] = useState<DashboardSearchState>(
    () =>
      parsePersistedDashboardView()?.searchByView ?? {
        owned: '',
        shared: '',
      },
  );
